import { applyFilters, CompositeUnit, LearningUnit, Path, PathUnit, Skill, Unit } from "../types";
 ...
 */
function availableActions<LU extends Unit>(currentState: State, learningUnits: ReadonlyArray<LU>) {
...
export function computeCost<LU extends Unit>(
    contextSwitchPenalty: number,
    lu: LU,
    subPath: Path | undefined,
    currentNode: SearchNode<LU>,
    suggestionViolationPenalty = true,
    fnCost: CostFunction<LU>
) {
    if ("selectors" in lu!) {
        return currentNode.cost + subPath?.cost! * 0.9;
    }
 ...
 */
export function searchV2<LU extends Unit>(
    initialState: State,
    goal: Skill[],
    ...
    while (openList.length > 0) {
        ...
        if (currentNode.state.goalFulfilled(goal)) {
            ...
            while (node.parent !== null) {
                ...
                if (lu) {
                    let pathUnit: PathUnit = {
                        unit: lu
                    };
                    if ("selectors" in lu! && node.subPath) {
                        pathUnit.path = node.subPath;
                    }
        ...
        for (const lu of availableActions(currentNode.state, learningUnits)) {
            let subPath: Path | undefined;
            if ("selectors" in lu!) {
                const lus = applyFilters(learningUnits.slice(), lu.selectors!).filter(
                    unit => unit.id != lu.id
                );
            ...
            const openGoals =
                goal.length > 1
                    ...
                    : goal;
            let heuristic = fnHeuristic(openGoals, lu);
            ...
            const newState = currentNode.state.deriveState(lu, subPath, globalKnowledge);
