...
export class Variable extends SkillExpression{
	...
	filterSkillsByWithout(skillsRelations: SkillsRelations, without?: Variable[]): SkillExpression[]{
		...
		if (childSkills.length > 0) {
			childSkills = childSkills.filter(
				childSkill => 
					without.every(variable => 
						!variable.evaluate([childSkill.id], skillsRelations, without))
			)
		} else {
		...
		childSkills.forEach(skill => {filteredTerms.push(new Variable(skill))});
