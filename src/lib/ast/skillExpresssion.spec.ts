...
describe("precondition formula", () => {
	...
	describe("AND", () => {
		...
		it("'and' complex skills expression with nested skills expressions precondition formula", () => {
			...
			const skillExpression = new And([
				new Or([new Variable(skill5), new Variable(skill6)]),
				new And([andSkills, orSkills])
			]);

			expect(skillExpression.evaluate(["skill:5", "skill:6"], skillsRelations)).toBeFalsy();
			expect(
	...
	describe("OR", () => {
		...
		it("'or' nested skill expression in precondition formula", () => {
			...
			const andSkillExpression = new Or([orSkills, andSkills]);

			expect(andSkillExpression.evaluate(["skill:1"], skillsRelations)).toBeTruthy();
		});
		...
		it("'or' complex skills expression with nested skills expressions precondition formula", () => {
			...
			const skillExpression = new Or([
				new Or([new Variable(skill5), new Variable(skill6)]),
				new And([andSkills, orSkills])
			]);

			expect(skillExpression.evaluate(["skill:5", "skill:6"], skillsRelations)).toBeTruthy();
			expect(
				skillExpression.evaluate(["skill:1", "skill:2", "skill:3"], skillsRelations)
			).toBeTruthy();
		});
	});
